...
func (r *ChatRepo) CreateGroup(ctx context.Context, groupID, ownerID, name, avatar, description string, memberLimit int32, now int64) error {
	...
	if err := r.dao.CreateChatGroup(ctx, chat.CreateChatGroupParams{
		GroupID:     gid,
		OwnerID:     oid,
		Name:        name,
		Avatar:      avatar,
...
func (r *ChatRepo) JoinGroup(ctx context.Context, groupID, userID string, now int64) error {
	...
	if _, e := qtx.GetChatGroupMemberRole(ctx, chat.GetChatGroupMemberRoleParams{GroupID: gid, UserID: uid}); e == nil {
	...
	if err := qtx.CreateChatGroupMember(ctx, chat.CreateChatGroupMemberParams{
		GroupID: gid,
		UserID:  uid,
		Role:    "member",
		Ctime:   now,
	...
	if _, err := qtx.IncChatGroupMemberCount(ctx, chat.IncChatGroupMemberCountParams{
		GroupID: gid,
		Utime:   now,
	}); err != nil {
		global.Log.Error(err)
...
func (r *ChatRepo) LeaveGroup(ctx context.Context, groupID, userID string, now int64) error {
	...
	role, err := qtx.GetChatGroupMemberRole(ctx, chat.GetChatGroupMemberRoleParams{GroupID: gid, UserID: uid})
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
	...
	aff, err := qtx.DeleteChatGroupMember(ctx, chat.DeleteChatGroupMemberParams{
		GroupID: gid,
		UserID:  uid,
	})
	if err != nil {
	...
	if _, err := qtx.DecChatGroupMemberCount(ctx, chat.DecChatGroupMemberCountParams{
		GroupID: gid,
		Utime:   now,
	}); err != nil {
		global.Log.Error(err)
...
func (r *ChatRepo) TransferOwner(ctx context.Context, groupID, ownerID, targetUserID string, now int64) error {
	...
	if _, err := qtx.GetChatGroupMemberRole(ctx, chat.GetChatGroupMemberRoleParams{GroupID: gid, UserID: tid}); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return ErrChatNotMember
		}
		global.Log.Error(err)
	...
	aff, err := qtx.TransferChatGroupOwner(ctx, chat.TransferChatGroupOwnerParams{
		GroupID:  gid,
		UserID:   tid,
		UserID_2: oid,
		Role:     "member",
	...
	if _, err := qtx.UpdateChatGroupOwnerID(ctx, chat.UpdateChatGroupOwnerIDParams{
		GroupID: gid,
		OwnerID: tid,
		Utime:   now,
	}); err != nil {
...
func (r *ChatRepo) UpdateMemberLastSeenTime(ctx context.Context, groupID, userID string, lastSeenTime int64) error {
	...
	aff, err := r.dao.UpdateChatGroupMemberLastSeenTime(ctx, chat.UpdateChatGroupMemberLastSeenTimeParams{
		GroupID:      gid,
		UserID:       uid,
...
func (r *ChatRepo) ListDiscoverGroups(ctx context.Context, userID, seed, cursorSortKey, cursorGroupID string, limit int) ([]ChatGroupDiscoverItem, string, bool, error) {
	...
	if cursorSortKey == "" && cursorGroupID == "" {
		rows, err := r.dao.ListDiscoverChatGroupsFirst(ctx, chat.ListDiscoverChatGroupsFirstParams{
			UserID:  uid,
			Column2: seed,
			Limit:   realLimit,
		})
		...
		rows, err := r.dao.ListDiscoverChatGroupsAfter(ctx, chat.ListDiscoverChatGroupsAfterParams{
			UserID:  uid,
			Column2: seed,
	...
	case []chat.ListDiscoverChatGroupsFirstRow:
	...
	case []chat.ListDiscoverChatGroupsAfterRow:
		items = make([]ChatGroupDiscoverItem, 0, len(v))
		for _, r := range v {
			items = append(items, ChatGroupDiscoverItem{
				GroupID:     r.GroupID,
...
func (r *ChatRepo) SearchGroupsByName(ctx context.Context, keyword string, limit int) ([]ChatGroupDiscoverItem, error) {
	...
	rows, err := r.dao.SearchChatGroupsByName(ctx, chat.SearchChatGroupsByNameParams{
...
func (r *ChatRepo) GetGroupProfile(ctx context.Context, groupID string, memberPreviewLimit int) (ChatGroupProfileItem, []ChatGroupMemberProfile, error) {
	...
	members, err := r.dao.ListChatGroupMembersPreviewWithProfile(ctx, chat.ListChatGroupMembersPreviewWithProfileParams{
		GroupID: gid,
...
func (r *ChatRepo) ListMembersWithProfile(ctx context.Context, groupID string, page, pageSize int) ([]ChatGroupMemberProfile, bool, error) {
	...
	rows, err := r.dao.ListChatGroupMembersWithProfile(ctx, chat.ListChatGroupMembersWithProfileParams{
		GroupID: gid,
		Limit:   limit,
		Offset:  offset,
	})
...
func (r *ChatRepo) SaveMessage(ctx context.Context, groupID, messageID, fromUserID, msgType, content string, now int64) error {
	...
	aff, err := r.dao.CreateChatGroupMessage(ctx, chat.CreateChatGroupMessageParams{
		MessageID:  mid,
		GroupID:    gid,
		FromUserID: uid,
		Type:       msgType,
...
func (r *ChatRepo) ListMessagesLatest(ctx context.Context, groupID string, limit int) ([]ChatGroupMessageItem, error) {
	...
	rows, err := r.dao.ListChatGroupMessagesLatest(ctx, chat.ListChatGroupMessagesLatestParams{
		GroupID: gid,
		Limit:   int32(limit),
	})
	if err != nil {
...
func (r *ChatRepo) ListMessagesBefore(ctx context.Context, groupID string, beforeCtime int64, beforeMessageID string, limit int) ([]ChatGroupMessageItem, error) {
	...
	rows, err := r.dao.ListChatGroupMessagesBefore(ctx, chat.ListChatGroupMessagesBeforeParams{
		GroupID:   gid,
		Ctime:     beforeCtime,
		MessageID: mid,
		Limit:     int32(limit),
...
func (r *ChatRepo) ListMessagesAfter(ctx context.Context, groupID string, afterCtime int64, afterMessageID string, limit int) ([]ChatGroupMessageItem, error) {
	...
	rows, err := r.dao.ListChatGroupMessagesAfter(ctx, chat.ListChatGroupMessagesAfterParams{
		GroupID:   gid,
		Ctime:     afterCtime,
		MessageID: mid,
		Limit:     int32(limit),
...
func (r *ChatRepo) GetMemberRole(ctx context.Context, groupID, userID string) (string, error) {
	...
	role, err := r.dao.GetChatGroupMemberRole(ctx, chat.GetChatGroupMemberRoleParams{
		GroupID: gid,
		UserID:  uid,
	})
	if err != nil {
