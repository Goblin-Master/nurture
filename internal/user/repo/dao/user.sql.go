...
const claimPendingUserEventOutbox = `-- name: ClaimPendingUserEventOutbox :many
WITH picked AS (
  SELECT e.id
...
type ClaimPendingUserEventOutboxParams struct {
	ClaimedAt   int64
	RetryBefore int64
	StaleBefore int64
	ClaimLimit  int32
...
type ClaimPendingUserEventOutboxRow struct {
	ID         int64
	EventID    string
	RoutingKey string
	Payload    string
...
func (q *Queries) ClaimPendingUserEventOutbox(ctx context.Context, arg ClaimPendingUserEventOutboxParams) ([]ClaimPendingUserEventOutboxRow, error) {
	rows, err := q.db.Query(ctx, claimPendingUserEventOutbox,
		arg.ClaimedAt,
		arg.RetryBefore,
		arg.StaleBefore,
		arg.ClaimLimit,
	...
	defer rows.Close()
	var items []ClaimPendingUserEventOutboxRow
	for rows.Next() {
		var i ClaimPendingUserEventOutboxRow
		if err := rows.Scan(
			&i.ID,
			&i.EventID,
			&i.RoutingKey,
...
const createUserEventOutbox = `-- name: CreateUserEventOutbox :execrows
...
type CreateUserEventOutboxParams struct {
	EventID    string
	RoutingKey string
	Payload    string
	Ctime      int64
...
func (q *Queries) CreateUserEventOutbox(ctx context.Context, arg CreateUserEventOutboxParams) (int64, error) {
	result, err := q.db.Exec(ctx, createUserEventOutbox,
		arg.EventID,
		arg.RoutingKey,
		arg.Payload,
		arg.Ctime,
...
const markUserEventOutboxFailed = `-- name: MarkUserEventOutboxFailed :execrows
...
type MarkUserEventOutboxFailedParams struct {
	ID          int64
	NextRetryAt int64
	Attempts    int32
	Utime       int64
...
func (q *Queries) MarkUserEventOutboxFailed(ctx context.Context, arg MarkUserEventOutboxFailedParams) (int64, error) {
	result, err := q.db.Exec(ctx, markUserEventOutboxFailed,
		arg.ID,
		arg.NextRetryAt,
		arg.Attempts,
		arg.Utime,
...
const markUserEventOutboxPublished = `-- name: MarkUserEventOutboxPublished :execrows
UPDATE "user_event_outbox"
...
type MarkUserEventOutboxPublishedParams struct {
	ID          int64
	PublishedAt int64
}

func (q *Queries) MarkUserEventOutboxPublished(ctx context.Context, arg MarkUserEventOutboxPublishedParams) (int64, error) {
	result, err := q.db.Exec(ctx, markUserEventOutboxPublished, arg.ID, arg.PublishedAt)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
